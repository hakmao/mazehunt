...
NCursesDisplay::NCursesDisplay(int winH, int winW) : win_height{winH}, win_width{winW}
{
    setup();
}
...
void NCursesDisplay::check_terminal_size()
{
    if (LINES < 24 || COLS < 80)
    {
        clear();
        endwin();
        std::cout << "Your terminal is too small: min [80 x 24], currently [" << COLS << " x " << LINES << "]"
                  << std::endl;
        exit(1);
    }
}
...
void NCursesDisplay::input_loop()
{
    ...
    do
    {
        ...
        case 's':
        case 'j':
            if ((player_pos.y < TEXT_LINES - 1) && is_move_okay(player_pos.y + 1, player_pos.x))
            {
                mvwaddch(win, player_pos.y, player_pos.x, CH_EMPTY | COLOR_PAIR(COL_EMPTY));
                player_pos.y += 1;
            }
        ...
        case 'd':
        case 'l':
            if ((player_pos.x < TEXT_COLS - 1) && is_move_okay(player_pos.y, player_pos.x + 1))
            {
                mvwaddch(win, player_pos.y, player_pos.x, CH_EMPTY | COLOR_PAIR(COL_EMPTY));
                player_pos.x += 1;
            }
